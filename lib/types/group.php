...
class group extends _type
{
	...
	public function renderItems($key, $data, $deleteButton=true)
	{
		...
		if($deleteButton) {
			...
			$value .= self::renderType(
				'',
				'divbutton::text=X:onclick=Vettich.Devform.GroupDelete(this);:template={content}:params=[title='.self::mess('#VDF_DELETE#').']'
			);
			$value .= $this->inlineTemplateItemEnd;
		}
	...
	public function renderAddButton()
	{
		...
		$value .= self::renderType(
			'', 
			'divbutton::text=#VDF_ADD#:onclick=Vettich.Devform.GroupAdd(this);:template={content}'
		);
		$value .= $this->inlineTemplateItemEnd;
		$value .= $this->inlineTemplateEnd;
		return $value;
